...
petRoutes.put("/pets/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!pet) {
      res.status(404).json({ error: "La mascota que intentas editar no existe." });
      return;
    }

    // 3. REGLA DE ORO: Autorización (¿Es el dueño?)
    if (pet.userId !== userId) {
      res.status(403).json({ error: "No tienes permiso para editar esta publicación. Solo el creador puede hacerlo." });
      return;
    }
    ...
    if (lat && lng) {
      ...
      if (isNaN(latNum) || isNaN(lngNum) || latNum < -90 || latNum > 90 || lngNum < -180 || lngNum > 180) {
        res.status(400).json({ error: "Las nuevas coordenadas geográficas son inválidas." });
        return;
      }
