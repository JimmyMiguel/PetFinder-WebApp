  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!pet) {
        res
          .status(404)
          .json({ error: "La mascota que intentas eliminar no existe." });
        return;
      }
      ...
      if (pet.userId !== userId) {
        res
          ...
          .json({
            error:
              "No tienes permiso para eliminar esta publicación. Solo el creador puede hacerlo.",
          });
        return;
      }
      ...
      if (pet.photos && pet.photos.length > 0) {
        try {
          ...
          console.error(
            "Error al borrar las fotos del storage, pero continuando con el borrado en BD:",
            storageError,
          );
        }
      }
      ...
      res
        .status(500)
        .json({ error: "Error interno del servidor al eliminar la mascota" });
    }
  },
);
